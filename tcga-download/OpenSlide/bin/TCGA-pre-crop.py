...
def crop_slide(img, save_slide_path, position=(0, 0), step=(0, 0), patch_size=224, multiscale=0): # position given as (y, x) at 5x scale
    if multiscale == 1:
    ...
    for s in tqdm(range(len(img_slides))):
        ...
        for j in range(step_y_max):
            for i in range(step_x_max):
                crop_slide(img, bag_path, (j*step_size, i*step_size), step=(j, i), patch_size=patch_size, multiscale=multiscale)
